...
function resolveBare(config: Config, addr: Address): Project[] {
  if (!config.primary || !config.zones[config.primary]) {
    throw new ResolveError(
      'no primary zone set — run `scaff -zone primary <name>` first, or use `scaff <zone>:<name>`.',
    );
  }
  return searchDirs(config.primary, config.zones[config.primary], addr.name);
}

function resolveInZone(config: Config, addr: Address): Project[] {
  const zone = addr.zone as string;
  const dirs = config.zones[zone];
  if (!dirs) {
  ...
  return searchDirs(zone, dirs, addr.name);
}

function searchDirs(zone: string, dirs: string[], name: string): Project[] {
